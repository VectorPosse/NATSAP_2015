---
title: "NATSAP- OQ diff (social problems) by RTC/WT and sex, plus dosage predictors"
author: "Sean Raleigh"
date: "August 19, 2015"
output: html_document
---

This document contains the analyis of OQ difference from the social problems subscale with RTC/WT and sex (and their interaction) as predictors, along with other group-level dosage predictors.
...
```{r}
# model name is 'OQdiff_soc_prob_RTCWT_by_sex_dosage'

RTCWT <- dose_tidy_z$RTCWT[natsap_tidy$new_ID]
RTCWT <- ifelse(RTCWT == "RTC", 0, 1)  # 0 = RTC, 1 = WT

data_OQdiff_soc_prob_RTCWT_by_sex_dosage <- list(y = natsap_tidy$OQdiff_soc_prob,
                                           n = length(natsap_tidy$OQdiff_soc_prob),
                                           J = NROW(dose_tidy_z),
...
",
file = "./OQdiff_soc_prob_RTCWT_by_sex_dosage/OQdiff_soc_prob_RTCWT_by_sex_dosage.stan")
```
...
if (file.exists("./OQdiff_soc_prob_RTCWT_by_sex_dosage/OQdiff_soc_prob_RTCWT_by_sex_dosage.rds") & final_model == TRUE) {
    stan_OQdiff_soc_prob_RTCWT_by_sex_dosage <-            readRDS("./OQdiff_soc_prob_RTCWT_by_sex_dosage/OQdiff_soc_prob_RTCWT_by_sex_dosage.rds")
} else {
    stan_OQdiff_soc_prob_RTCWT_by_sex_dosage <- stan_model("./OQdiff_soc_prob_RTCWT_by_sex_dosage/OQdiff_soc_prob_RTCWT_by_sex_dosage.stan")
    saveRDS(stan_OQdiff_soc_prob_RTCWT_by_sex_dosage, file = "./OQdiff_soc_prob_RTCWT_by_sex_dosage/OQdiff_soc_prob_RTCWT_by_sex_dosage.rds")
}

fit_OQdiff_soc_prob_RTCWT_by_sex_dosage <- sampling(stan_OQdiff_soc_prob_RTCWT_by_sex_dosage, data = data_OQdiff_soc_prob_RTCWT_by_sex_dosage)
```

Extract and save samples:

```{r}
samples_OQdiff_soc_prob_RTCWT_by_sex_dosage <- as.data.frame(fit_OQdiff_soc_prob_RTCWT_by_sex_dosage)

write_csv(samples_OQdiff_soc_prob_RTCWT_by_sex_dosage, path = "./OQdiff_soc_prob_RTCWT_by_sex_dosage/OQdiff_soc_prob_RTCWT_by_sex_dosage.csv")

# Get 5 random programs
set.seed(1234)
some_programs <- sort(sample(1:data_OQdiff_soc_prob_RTCWT_by_sex_dosage$J, 5))
...
params_OQdiff_soc_prob_RTCWT_by_sex_dosage <- c("mu_a", "sigma_a",
                                          # alphas,
                                          "female", "WT", "WT_female",
                                          "g_IT_freq", "g_GT_freq",
...
traceplot(fit_OQdiff_soc_prob_RTCWT_by_sex_dosage, pars = params_OQdiff_soc_prob_RTCWT_by_sex_dosage, inc_warmup = FALSE)

print(fit_OQdiff_soc_prob_RTCWT_by_sex_dosage)

len <- length(params_OQdiff_soc_prob_RTCWT_by_sex_dosage)
sample_plots(samples_OQdiff_soc_prob_RTCWT_by_sex_dosage,
             params = params_OQdiff_soc_prob_RTCWT_by_sex_dosage,
             cred_mass = 0.95,
...
```{r, results = 'hide'}
cleanup("OQdiff_soc_prob_RTCWT_by_sex_dosage")
