---
title: "NATSAP- OQ diff (interpersonal relations) by RTC/WT and sex, plus dose predictors"
author: "Sean Raleigh"
date: "August 19, 2015"
output: html_document
---

This document contains the analyis of OQ difference from the interpersonal relations subscale with RTC/WT and sex (and their interaction) as predictors, along with other group-level dose predictors.
...
```{r}
# model name is 'OQdiff_int_rel_RTCWT_by_sex_dose'

RTCWT <- dose_tidy_z$RTCWT[natsap_tidy$new_ID]
RTCWT <- ifelse(RTCWT == "RTC", 0, 1)  # 0 = RTC, 1 = WT

data_OQdiff_int_rel_RTCWT_by_sex_dose <- list(y = natsap_tidy$OQdiff_int_rel,
...
",
file = "./OQdiff_int_rel_RTCWT_by_sex_dose/OQdiff_int_rel_RTCWT_by_sex_dose.stan")
```
...
if (file.exists("./OQdiff_int_rel_RTCWT_by_sex_dose/OQdiff_int_rel_RTCWT_by_sex_dose.rds") & final_model == TRUE) {
    stan_OQdiff_int_rel_RTCWT_by_sex_dose <-            readRDS("./OQdiff_int_rel_RTCWT_by_sex_dose/OQdiff_int_rel_RTCWT_by_sex_dose.rds")
} else {
    stan_OQdiff_int_rel_RTCWT_by_sex_dose <- stan_model("./OQdiff_int_rel_RTCWT_by_sex_dose/OQdiff_int_rel_RTCWT_by_sex_dose.stan")
    saveRDS(stan_OQdiff_int_rel_RTCWT_by_sex_dose, file = "./OQdiff_int_rel_RTCWT_by_sex_dose/OQdiff_int_rel_RTCWT_by_sex_dose.rds")
}

fit_OQdiff_int_rel_RTCWT_by_sex_dose <- sampling(stan_OQdiff_int_rel_RTCWT_by_sex_dose, data = data_OQdiff_int_rel_RTCWT_by_sex_dose)
```

Extract and save samples:

```{r}
samples_OQdiff_int_rel_RTCWT_by_sex_dose <- as.data.frame(fit_OQdiff_int_rel_RTCWT_by_sex_dose)

write_csv(samples_OQdiff_int_rel_RTCWT_by_sex_dose, path = "./OQdiff_int_rel_RTCWT_by_sex_dose/OQdiff_int_rel_RTCWT_by_sex_dose.csv")

# Get 5 random programs
set.seed(1234)
some_programs <- sort(sample(1:data_OQdiff_int_rel_RTCWT_by_sex_dose$J, 5))
...
params_OQdiff_int_rel_RTCWT_by_sex_dose <- c("mu_a", "sigma_a",
                                          # alphas,
                                          "female", "WT", "WT_female",
                                          "g_IT", "g_GT",
                                          "g_RFT", "g_IPFT",
...
traceplot(fit_OQdiff_int_rel_RTCWT_by_sex_dose, pars = params_OQdiff_int_rel_RTCWT_by_sex_dose, inc_warmup = FALSE)

print(fit_OQdiff_int_rel_RTCWT_by_sex_dose)

len <- length(params_OQdiff_int_rel_RTCWT_by_sex_dose)
sample_plots(samples_OQdiff_int_rel_RTCWT_by_sex_dose,
             params = params_OQdiff_int_rel_RTCWT_by_sex_dose,
             cred_mass = 0.95,
...
```{r, results = 'hide'}
cleanup("OQdiff_int_rel_RTCWT_by_sex_dose")
