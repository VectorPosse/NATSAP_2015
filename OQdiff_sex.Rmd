...
cat("
    ...
    model {
        // Implicit uniform priors on b_0, b_sex, and sigma
...
params_OQdiff_sex <- c("b_0", "b_sex", "sigma")
```
...
cat("
    ...
    model {
        // Implicit uniform priors on b_sex, mus, and sigmas
        for (i in 1:n) {
...
params_OQdiff_sex_prog <- c(mus, "b_sex", sigmas)
```
...
params_OQdiff_sex_prog_hier <- c("mu_a", "sigma_a", "b_sex",  "sigma_y", alphas)
```
